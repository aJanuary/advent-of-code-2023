...
CARD_SORT = ['J', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A']
...
def parse_hand(hand)
  ...
  num_jokers = (by_card['J'] || []).size
  if by_card.size == 1 || (by_card.size == 2 && num_jokers > 0)
    Hand.new(:five_of_a_kind, cards)
  elsif by_card.size == 2 || (by_card.size == 3 && num_jokers > 0)
    if by_card.values.any? {|x| x.size == 4 - num_jokers}
      Hand.new(:four_of_a_kind, cards)
    else
      Hand.new(:full_house, cards)
    end
  elsif by_card.size == 3 || (by_card.size == 4 && num_jokers > 0)
    if by_card.values.any? {|x| x.size == 3 - num_jokers }
      Hand.new(:three_of_a_kind, cards)
    else
      Hand.new(:two_pairs, cards)
    end
  elsif by_card.size == 4 || num_jokers > 0
